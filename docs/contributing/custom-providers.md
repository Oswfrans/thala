...
Thala supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export THALA_API_KEY="your-api-key"
thala agent
```

## llama.cpp Server (Recommended Local Setup)

Thala includes a first-class local provider for `llama-server`:
...
```bash
thala models refresh --provider llamacpp
thala agent -m "hello"
```

You do not need to export `THALA_API_KEY=dummy` for this flow.

## SGLang Server

Thala includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
thala models refresh --provider sglang
thala agent -m "hello"
```
...
Thala includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
thala models refresh --provider vllm
thala agent -m "hello"
```
