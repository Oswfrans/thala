# Thala PR Workflow (High-Volume Collaboration)

This document defines how Thala handles high PR volume while maintaining:
...
- For `.github/workflows/**`, require explicit maintainer review and CODEOWNERS approval.
...
- Contributor opens PR with full `.github/pull_request_template.md`.
- Apply risk/size/scope labels manually during triage when automation is unavailable.

### 4.2 Step B: Validation

- `CI Required Gate` is the merge gate.
- Docs-only PRs use fast-path and skip heavy Rust jobs.
- Non-doc PRs must pass lint, strict delta lint, tests, build matrix, security checks, and compatibility checks.
...
- If identity-like wording appears in tests/examples, it is normalized to Thala/project-native labels.
...
- Labeling may be manual; reviewers should apply `size:*` consistently based on effective changed lines.
- Docs-only/lockfile-heavy PRs should be normalized to avoid size inflation.
...
- Maintain queue hygiene manually when stale automation is unavailable.
- During triage, prioritize PRs that are behind `master` or missing/failing `CI Required Gate` on the head commit.
...
- Avoid duplicate automated comments during triage.
- Any auto-close behavior must stay limited to issues, not PRs.
- Maintainers can use manual risk labels when context demands human override.
...
- Confirm any required identity-like wording uses Thala/project-native terminology.
