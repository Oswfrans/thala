...
```bash
# Check config
cat ~/.thala/config.toml | grep -A 5 telegram
...
| Health check | <5s | `time thala channel doctor` |
...
5. **Monitor logs** for any issues:
   ...
   thala daemon
   # or
   RUST_LOG=info thala channel start
   ```
...
- Issues: https://github.com/oswfrans/thala/issues
