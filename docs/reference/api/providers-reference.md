...
3. Generic fallback env vars: `THALA_API_KEY` then `API_KEY`
...
- After multimodal normalization, Thala sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, Thala returns a structured capability error instead of silently ignoring images.
...
- Thala normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `thala models refresh --provider llamacpp`
...
- Model discovery: `thala models refresh --provider sglang`
...
- Model discovery: `thala models refresh --provider vllm`
...
- Model discovery: `thala models refresh --provider osaurus`
...
- Model discovery: `thala models refresh --provider nvidia`
