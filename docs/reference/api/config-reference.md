# Thala Config Reference (Operator-Oriented)
...
1. `THALA_WORKSPACE` override (if set)
2. persisted `~/.thala/active_workspace.toml` marker (if present)
3. default `~/.thala/config.toml`

Thala logs the resolved config on startup at `INFO` level:

- `Config loaded` with fields: `path`, `workspace`, `source`, `initialized`

Schema export command:

- `thala config schema` (prints JSON Schema draft 2020-12 to stdout)
...
| `otel_service_name` | `thala` | Service name emitted to OTLP collector |
...
- Query runtime traces with:
  - `thala doctor traces --limit 20`
  - `thala doctor traces --event tool_call_result --contains \"error\"`
  - `thala doctor traces --id <trace-id>`
...
1. `THALA_PROVIDER` (explicit override, always wins when non-empty)
...
- Use `THALA_PROVIDER` when you intentionally want runtime env to override a non-default configured provider.
...
- When `enabled = true` and no OTP secret exists, Thala generates one and prints an enrollment URI once.
...
| `state_file` | `~/.thala/estop-state.json` | Persistent estop state path |
...
- Use CLI command `thala estop` to engage and `thala estop resume` to clear levels.
...
- Security-first default: Thala does **not** clone or sync `open-skills` unless `open_skills_enabled = true`.
- Environment overrides:
  - `THALA_OPEN_SKILLS_ENABLED` accepts `1/0`, `true/false`, `yes/no`, `on/off`.
  - `THALA_OPEN_SKILLS_DIR` overrides the repository path when non-empty.
  - `THALA_SKILLS_PROMPT_MODE` accepts `full` or `compact`.
- Precedence for enable flag: `THALA_OPEN_SKILLS_ENABLED` → `skills.open_skills_enabled` in `config.toml` → default `false`.
...
- Skill loading and `thala skills install` both apply a static security audit. Skills that contain symlinks, script-like files, high-risk shell payload snippets, or unsafe markdown link traversal are rejected.
...
| `allowed_domains` | `[]` | Allowed domains for browser open/navigation actions (exact/subdomain match, or `"*"` for all public domains) |
...
3. Validate with `thala doctor` before restart/rollout.
...
- While `thala channel start` is running, updates to `default_provider`, `default_model`, `default_temperature`, `api_key`, `api_url`, and `reliability.*` are hot-applied from `config.toml` on the next inbound message.
...
- `THALA_LINQ_SIGNING_SECRET` overrides `signing_secret` when set.
...
- `THALA_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides `webhook_secret` when set.
