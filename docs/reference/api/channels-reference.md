...
This document is the canonical reference for channel configuration in Thala.
...
6. **Stale runtime state**: config changed but `thala daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.thala/config.toml`.
...
When running `thala channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `thala models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

Thala supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `thala channel list`, `thala channel doctor`, and `thala channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@thala:matrix.example.com"   # optional, recommended for E2EE
...
Thala supports two WhatsApp backends:
...
```toml
[channels_config.whatsapp]
session_path = "~/.thala/state/whatsapp-web/session.db"
...
- `THALA_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `THALA_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `thala daemon` after config changes.
...
```bash
RUST_LOG=info thala daemon 2>&1 | tee /tmp/thala.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel" /tmp/thala.log
```
