...
Thala routes MQTT/webhook/cron events through a unified SOP dispatcher (`dispatch_sop_event`).
...
| **Cron not firing** | daemon not running or invalid expression | run `thala daemon`; check logs for cron parse warnings |
