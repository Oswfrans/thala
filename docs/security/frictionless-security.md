...
[INFO] ✓ Audit logging enabled (~/.config/thala/audit.log)
...
```bash
# Check what's active
$ thala security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/thala/audit.log
  → 47 events logged today

# Disable sandbox explicitly (writes to config)
$ thala config set security.sandbox.enabled false

# Enable specific backend
$ thala config set security.sandbox.backend firejail

# Adjust limits
$ thala config set security.resources.max_memory_mb 2048
```
...
✅ **Status visibility** — `thala security --status`
