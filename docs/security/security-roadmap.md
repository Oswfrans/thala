# Thala Security Improvement Roadmap
...
Thala already has **excellent application-layer security**:
...
## Comparison: Thala vs PicoClaw vs Production Grade

| Feature | PicoClaw | Thala Now | Thala + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`thala audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/thala/audit.log"
sign_events = true
max_size_mb = 100
...
thala security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
thala audit --user @alice --since 24h
thala audit --risk high --violations-only
thala audit --verify-signatures
...
**Thala is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, Thala becomes:
...
**Value**: Transforms Thala from "safe for testing" to "safe for production"
