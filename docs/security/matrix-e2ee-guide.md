...
This guide explains how to run Thala reliably in Matrix rooms, including end-to-end encrypted (E2EE) rooms.
...
3. `room_id` is correct:
   ...
   - supported: room alias (`#alias:server`) and Thala will resolve it
...
user_id = "@thala:matrix.example.com"
...
- Thala attempts to read identity from Matrix `/_matrix/client/v3/account/whoami`.
...
3. Confirm Thala logs contain Matrix listener startup and no repeated sync/auth errors.
...
- Thala sends Matrix text replies as markdown-capable `m.room.message` text content.
- Matrix clients that support `formatted_body` should render emphasis, lists, and code blocks.
- If formatting appears as plain text, check client capability first, then confirm Thala is running a build that includes markdown-enabled Matrix output.
