...
| Tình huống | Lệnh |
|----------|---------|
| Có API key, muốn cài nhanh nhất | `thala onboard --api-key sk-... --provider openrouter` |
| Muốn được hướng dẫn từng bước | `thala onboard` |
| Đã có config, chỉ cần sửa kênh | `thala onboard --channels-only` |
...
- Thiết lập nhanh: `thala onboard --api-key "sk-..." --provider openrouter`
- Thiết lập hướng dẫn: `thala onboard`
- Kiểm tra môi trường: `thala status` + `thala doctor`
