...
This guide covers native Nextcloud Talk integration for Thala.
...
Add this section in `~/.thala/config.toml`:
...
thala daemon
# or
thala gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, Thala verifies:
...
- Thala ignores bot-originated webhook events (`actorType = bots`).
- Thala ignores non-message/system events.
...
3. Confirm Thala receives and replies in the same room.
