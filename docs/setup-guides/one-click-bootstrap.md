...
This page defines the fastest supported path to install and initialize Thala.
...
Default behavior is **app-only** (build/install Thala) and expects existing Rust toolchain.
...
```bash
curl -fsSL https://raw.githubusercontent.com/oswfrans/thala/master/install.sh | bash
```
...
This builds a local Thala image and launches onboarding inside a container while
persisting config/workspace to `./.thala-docker`.

Container CLI defaults to `docker`. If Docker CLI is unavailable and `podman` exists,
the installer auto-falls back to `podman`. You can also set `THALA_CONTAINER_CLI`
explicitly (for example: `THALA_CONTAINER_CLI=podman ./install.sh --docker`).
...
Docker tag (`THALA_DOCKER_IMAGE`, default: `thala-bootstrap:local`); if missing,
it pulls `ghcr.io/oswfrans/thala:latest` and tags it locally before running.
...
are persisted in the data directory (default: `./.thala-docker`, or `~/.thala-docker`
when bootstrapping via `curl | bash`). You can override this path with `THALA_DOCKER_DATA_DIR`.
...
The simplest way to run Thala long-term in Docker/Podman is with the provided
`docker-compose.yml` at the repository root. It uses a named volume (`thala-data`)
...
If you installed via `./install.sh --docker` and want to reuse the `.thala-docker`
...
podman run -d --name thala \
  --restart unless-stopped \
  --userns keep-id \
  --user "$(id -u):$(id -g)" \
  -v "$PWD/.thala-docker/.thala:/thala-data/.thala:Z" \
  -v "$PWD/.thala-docker/workspace:/thala-data/workspace:Z" \
  -e HOME=/thala-data \
  -e THALA_WORKSPACE=/thala-data/workspace \
  -p 42617:42617 \
  thala-bootstrap:local \
...
# Remove the container (data in volumes/.thala-docker is preserved)
docker rm thala
...
saved in `.thala-docker/.thala/config.toml` and do not need to be passed again.
...
```bash
THALA_API_KEY="sk-..." THALA_PROVIDER="openrouter" ./install.sh
```
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/oswfrans/thala:latest`)
- `--skip-install`
- `--provider <id>`
