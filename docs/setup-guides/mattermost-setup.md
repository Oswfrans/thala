...
Thala supports native integration with Mattermost via its REST API v4. This integration is ideal for self-hosted, private, or air-gapped environments where sovereign communication is a requirement.
...
| `mention_only` | (Optional) When `true`, only messages that explicitly mention the bot username (for example `@thala-bot`) are processed. Default: `false`. |

## Threaded Conversations

Thala supports Mattermost threads in both modes:
- If a user sends a message in an existing thread, Thala always replies within that same thread.
...
When `mention_only = true`, Thala applies an extra filter after `allowed_users` authorization:
