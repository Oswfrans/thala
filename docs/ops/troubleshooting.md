...
- `cargo check` / `cargo build` appears stuck at `Checking thala` for a long time
- repeated `Blocking waiting for file lock on package cache` or `build directory`

Why this happens in Thala:
...
### `thala` command not found after install

Symptom:

- install succeeds but shell cannot find `thala`
...
- stop extra `thala daemon` / `thala channel start` processes
