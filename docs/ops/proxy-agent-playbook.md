...
- **Audience:** operators and maintainers running Thala in proxied networks.
- **Scope:** `proxy_config` actions, mode selection, verification flow, and troubleshooting.
- **Non-goals:** generic network debugging outside Thala runtime behavior.
...
### 1.1 Proxy only Thala internal traffic
...
- [Section 4](#4-mode-a--proxy-only-for-thala-internals)
...
|---|---|---|---|
| `thala` | Thala internal HTTP clients | No | Normal runtime proxying without process-level side effects |
...
## 4. Mode A — Proxy Only for Thala Internals

Use when Thala provider/channel/tool HTTP traffic should use proxy, without exporting process-level proxy env vars.

Tool calls:

```json
{"action":"set","enabled":true,"scope":"thala","http_proxy":"http://127.0.0.1:7890","https_proxy":"http://127.0.0.1:7890","no_proxy":["localhost","127.0.0.1"]}
{"action":"get"}
```

Expected behavior:

- Runtime proxy is active for Thala HTTP clients.
