#!/bin/bash
# Thala Telegram Integration Test Suite
...
if [ -f "target/release/thala" ]; then
    BINARY_SIZE=$(ls -lh target/release/thala | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/thala 2>/dev/null || stat -c%s target/release/thala)
...
CONFIG_PATH="$HOME/.thala/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'thala onboard' first"
    fi
else
    warn "No config file found - run 'thala onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/thala channel doctor 2>&1 || true)
END_TIME=$(date +%s)
