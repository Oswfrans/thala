...
This guide covers testing the Telegram channel integration for Thala.
...
6. **Error logging**
    ...
    ```bash
    RUST_LOG=debug thala channel start
    ```
...
Solution: Check user allowlist
  ...
  4. Run: thala onboard --channels-only
```
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time thala channel doctor`   |
| First response time    | <3s        | Time from sending to receiving   |
| Message split overhead | <50ms      | Check logs for timing            |
| Memory usage           | <10MB      | `ps aux \| grep thala`        |
| Binary size            | ~3-4MB     | `ls -lh target/release/thala` |
...
```bash
# Maximum logging
RUST_LOG=trace thala channel start
...
# Validate config
cat ~/.thala/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
- [Thala Main README](../../README.md)
- [Contributing Guide](../../CONTRIBUTING.md)
- [Issue Tracker](https://github.com/oswfrans/thala/issues)
