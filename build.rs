...
fn main() {
    ...
    println!("cargo:rerun-if-changed=docs/assets/thala-trans.png");
...
fn ensure_dashboard_assets(dist_dir: &Path) {
    ...
    let src = Path::new("docs/assets/thala-trans.png");
    if !src.exists() {
        eprintln!(
            "cargo:warning=docs/assets/thala-trans.png not found; skipping dashboard asset copy"
        );
        return;
    }

    let dst = dist_dir.join("thala-trans.png");
    if let Err(e) = fs::copy(src, &dst) {
        eprintln!("cargo:warning=Failed to copy thala-trans.png into web/dist/: {e}");
    }
}
