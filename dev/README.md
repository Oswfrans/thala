# Thala Development Environment

A fully containerized development sandbox for Thala agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`thala`)

```bash
./dev/cli.sh agent
```

Use this to run `thala` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/thala-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.thala` (mounted to `/thala-data/.thala`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.thala` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- `./dev/scripts/ci/rust_quality_gate.sh`
...
./dev/scripts/ci/docs_quality_gate.sh
# Optional host-side docs links gate (changed-line added links)
./dev/scripts/ci/docs_links_gate.sh
```
...
- Rust compilation, tests, and audit/deny tools run in `thala-local-ci` container.
...
- Both `dev/docker/Dockerfile` and `dev/ci/Dockerfile` use BuildKit cache mounts for Cargo registry/git data.
- The root `Dockerfile` also caches Rust `target/` (`id=thala-target`) to speed repeat local image builds.
