...
TAG="v${VERSION}"
ASSET_NAME="thala-${TARGET}.tar.gz"
ASSET_URL="https://github.com/oswfrans/thala/releases/download/${TAG}/${ASSET_NAME}"
TEMP_DIR="$(mktemp -d -t thala-termux-test-XXXXXX)"
...
# --- Test 5: Contains thala binary ---
info "Checking archive contents"
CONTENTS=$(tar -tzf "$TEMP_DIR/$ASSET_NAME")
if echo "$CONTENTS" | grep -q "^thala$"; then
  pass "Archive contains 'thala' binary"
else
  fail "Archive does not contain 'thala' binary"
  echo "Contents:"
  echo "$CONTENTS" | sed 's/^/  /'
fi
...
CHECKSUMS_URL="https://github.com/oswfrans/thala/releases/download/${TAG}/SHA256SUMS"
