...
build_smoke_image() {
  if docker buildx version >/dev/null 2>&1; then
    ...
    DOCKER_BUILDKIT=1 docker build --target dev -t thala-local-smoke:latest .
  fi
}

print_help() {
  cat <<'EOF'
Thala Local CI in Docker
...
Commands:
  ...
  validate      Run deterministic validation test suite (container only)
...
case "$1" in
  ...
  lint)
    run_in_ci "./dev/scripts/ci/rust_quality_gate.sh"
    ;;

  lint-strict)
    run_in_ci "./dev/scripts/ci/rust_quality_gate.sh --strict"
    ;;

  lint-delta)
    run_in_ci "./dev/scripts/ci/rust_strict_delta_gate.sh"
    ;;
  ...
  validate)
    run_in_ci "cargo test --locked --verbose --lib --tests"
    ;;
  ...
  docker-smoke)
    build_smoke_image
    docker run --rm thala-local-smoke:latest --version
    ;;
