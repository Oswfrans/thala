...
## Protected Paths Checklist (required for autonomous merge)

Autonomous merges to `master` are blocked if PR touches protected paths. Verify:

- [ ] This PR does NOT touch protected paths (unchecked = blocks auto-merge):
  - [ ] No changes to `auth/**` (authentication/authorization)
  - [ ] No changes to `billing/**` (billing/payments)
  - [ ] No changes to `infra/**` (infrastructure/deployment)
  - [ ] No changes to `**/migrations/**` (database migrations)
  - [ ] No changes to `.github/workflows/**` (CI/CD pipelines)
- [ ] This is NOT a change to the Thala repository (`thala-core`) - Thala never auto-merges self-changes

If any box is unchecked, this PR requires human review and manual merge.

## Privacy and Data Hygiene (required)

- Data-hygiene status (`pass|needs-follow-up`):
- Redaction/anonymization notes:
- Neutral wording confirmation (use Thala/project-native labels if identity-like wording is needed):
