...
2. **GitHub**: Use [GitHub Security Advisories](https://github.com/oswfrans/thala/security/advisories/new)
...
Thala implements defense-in-depth security:
...
Thala Docker images follow CIS Docker Benchmark best practices:
...
docker build -t thala .
docker inspect --format='{{.Config.User}}' thala
# Expected: 65534:65534

# Run with read-only filesystem (production hardening)
docker run --read-only -v /path/to/workspace:/workspace thala gateway
```
